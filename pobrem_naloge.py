...
vzorec_filma = re.compile(
    ...
    #r'<a href="/title/tt(?P<id>\d+)/.*?lister-item-year text-muted unbold">\((?P<leto>\d{4})\)'
...
def filmi_na_strani(st_strani, na_stran=250):
    ...
    ime_datoteke = 'Projektna_Naloga/zajeti-podatki/najbolj-znani-filmi-{}.html'.format(st_strani)
...
filmi = []
for st_strani in range(1, 2):
    for film in filmi_na_strani(st_strani, 250):
        filmi.append(film)
filmi.sort(key=lambda film: film['id'])
orodja.zapisi_json(filmi, 'Projektna_Naloga/obdelani-podatki/filmi.json')
osebe, vloge, zanri = izloci_gnezdene_podatke(filmi)
orodja.zapisi_csv(
    filmi,
    ['id', 'naslov', 'dolzina', 'leto', 'ocena', 'metascore', 'glasovi', 'zasluzek', 'oznaka', 'opis'], 'Projektna_Naloga/obdelani-podatki/filmi.csv'
)
orodja.zapisi_csv(osebe, ['id', 'ime'], 'Projektna_Naloga/obdelani-podatki/osebe.csv')
orodja.zapisi_csv(vloge, ['film', 'oseba', 'vloga', 'mesto'], 'Projektna_Naloga/obdelani-podatki/vloge.csv')
orodja.zapisi_csv(zanri, ['film', 'zanr'], 'Projektna_Naloga/obdelani-podatki/zanri.csv')
